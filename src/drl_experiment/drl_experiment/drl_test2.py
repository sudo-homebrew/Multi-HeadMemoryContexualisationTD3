...
class SensorSub(Node):
    ...
    *******************************************************************************"""
    ...
    def odom_callback(self, msg):
        ...
        self.current_pose_w = msg.pose.pose.orientation.w
    ...
    def obstacle_odom_callback(self, msg):
        if 'obstacle' in msg.child_frame_id:
            ...
            diff_x = self.current_pose_x - robot_pos.x
            diff_y = self.current_pose_y - robot_pos.y
    ...
    *******************************************************************************"""
    ...
    def social_reward(self):
        robot_polygon = self.create_polygon(self.current_pose_x, self.current_pose_y, self.current_pose_w)
        obstacles_polygon = []
...
class Experiment(Node):
    ...
    def process_step(self):
        ...
        self.loop_cnt += 1
        if min(self.sub.obstacle_distances) < 1:
            self.echo_cnt += 1
        if self.sub.social_reward():
            self.social_cnt += 1

        self.distance_travled += self.sub.distance_from_last_step()
        robot_pose_world = self.sub.get_current_robot_world_pose()

        self.write_to_csv(self.result_directory, "Echo_Violation.log", self.echo_cnt / self.loop_cnt)
        self.write_to_csv(self.result_directory, "Social_Violation.log", self.social_cnt / self.loop_cnt)
    ...
    def finish_experiment(self):
        ...
        file_path = os.path.join(self.result_directory, 'Outcomes_duration.log')
