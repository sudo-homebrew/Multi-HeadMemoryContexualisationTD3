...
class Actor(Network):
    def __init__(self, name, state_size, action_size, hidden_size):
        ...
        self.conv_fc = nn.Linear((state_size - 6) * output_size, int(hidden_size / 2 ** 2))
    ...
    def forward(self, states, visualize=False):
        ...
        feature = torch.sigmoid(self.conv_fc(torch.flatten(self.unet(lidar_features), start_dim=cat_dim)))

        opx = self.silu(self.opfa1(env_states))
        opx = self.silu(self.opfa2(opx))

        x = torch.cat((x, feature, opx), cat_dim)
        # x = torch.cat((feature, opx), cat_dim)
...
class Critic(Network):
    def __init__(self, name, state_size, action_size, hidden_size):
        ...
        self.q1conv_fc = nn.Linear((state_size - 6) * output_size, int(hidden_size / 6))
        ...
        self.q2conv_fc = nn.Linear((state_size - 6) * output_size, int(hidden_size / 6))
    ...
    def forward(self, states, actions):
        ...
        xf = torch.sigmoid(self.q1conv_fc(torch.flatten(self.q1unet(lidar_features), start_dim=cat_dim)))
        ...
        xf = torch.sigmoid(self.q2conv_fc(torch.flatten(self.q2unet(lidar_features), start_dim=cat_dim)))
