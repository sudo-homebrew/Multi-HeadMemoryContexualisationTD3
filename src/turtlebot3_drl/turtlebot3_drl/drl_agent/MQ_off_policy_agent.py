...
from ..common.settings import ENABLE_BACKWARD, ENABLE_STACKING, ACTION_SIZE, HIDDEN_SIZE, BATCH_SIZE, BUFFER_SIZE, DISCOUNT_FACTOR, \
                                 LEARNING_RATE, TAU, STEP_TIME, EPSILON_DECAY, EPSILON_MINIMUM, STACK_DEPTH, FRAME_SKIP, MQ_SIZE
...
class OffPolicyAgent(ABC):
    def __init__(self, device, simulation_speed):
        ...
        # Network structure
        self.state_size         = NUM_SCAN_SAMPLES + 6 # Could add more states on environment.py(also environment_real.py)
    ...
    def _train(self, replaybuffer):
        ...
        sample_s, sample_a, sample_r, sample_ns, sample_d, sample_m, sample_nm = batch
        sample_m = torch.from_numpy(sample_m).to(self.device)
        sample_nm = torch.from_numpy(sample_nm).to(self.device)
        ...
        result = self.train(sample_m, sample_s, sample_a, sample_r, sample_ns, sample_nm, sample_d)
        self.iteration += 1
    ...
    def create_network(self, type, name):
        network = type(name, self.mq_size, self.input_size, self.action_size, self.hidden_size).to(self.device)
