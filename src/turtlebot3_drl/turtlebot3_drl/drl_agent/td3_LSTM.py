...
class Actor(Network):
    def __init__(self, name, state_size, action_size, hidden_size):
        ...
        self.lstm3 = nn.LSTM(input_size=hidden_size // 2 ** 2, hidden_size=action_size, num_layers=1)
        # self.fa = nn.Linear(hidden_size // 2 ** 2, action_size)
    ...
    def forward(self, states, visualize=False):
        ...
        if cat_dim == 0:
            action = torch.tanh(lx)[0, 0, :]
        else:
            action = torch.tanh(lx)[0, :, :]

        # action = torch.tanh(self.fa(lx))
