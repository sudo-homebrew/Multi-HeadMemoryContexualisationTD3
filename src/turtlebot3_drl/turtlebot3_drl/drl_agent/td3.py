...
class Actor(Network):
    def __init__(self, name, state_size, action_size, hidden_size):
        ...
        self.state_history = TensorQueue(500, state_size, self.dev) #queue.Queue(hidden_size)
        # --- define layers here ---
        ...
        self.fa5 = nn.Linear(state_size + hidden_size + 6, int(hidden_size / 2 ** 1))
        self.fa6 = nn.Linear(int(hidden_size / 2 ** 1 + 6), int(hidden_size / 2 ** 2))
        self.fa7 = nn.Linear(int(hidden_size / 2 ** 2 + 6), int(hidden_size / 2 ** 3))
        ...
        fc_size = 704 #state_size // (self.pooling_kernel_size ** self.conv_iter) * inner_channel_size

        self.conv = nn.Sequential(
            nn.Conv1d(1, inner_channel_size, 4, padding='same', padding_mode='circular'),
            # nn.BatchNorm1d(inner_channel_size),
            nn.SiLU(),
...
class Critic(Network):
    ...
    def state_filter(self, state):
        if len(state.size()) == 2 and state.size()[1] == 184:
            cat_dim = 1
            lidar_states = state[:, :self.state_size]
        ...
        elif len(state.size()) == 1 and state.size()[0] == 184:
            cat_dim = 0
            lidar_states = state[:self.state_size]
            env_states = (state[-4:] + 1) / 2
            lidar_states = (torch.exp((torch.ones_like(lidar_states) - lidar_states) * 4) - 1) / (torch.exp(torch.ones_like(lidar_states) * 4) - 1)
...
class TD3(OffPolicyAgent):
    def __init__(self, device, sim_speed):
        ...
        if ENABLE_IMITATE_ACTION:
            self.sm = StorageManager('ddpg', 'ddpg_14_stage_10', 15400, self.device, '10')
    ...
    def sensor_value_addition(self, lidar_sensor, input_diff, device):
        lidar_sensor = lidar_sensor.tolist()
        initial_length = len(lidar_sensor)
        
        for i in range(initial_length):
            if i * 2 + 1 < initial_length * 2:
                new_value = (lidar_sensor[i] + lidar_sensor[(i + 1) % initial_length]) / 2
                lidar_sensor.insert(i * 2 + 1, new_value)

        return torch.tensor(lidar_sensor).to(device)
    ...
    def get_action_imitate(self, state):
        ...
        r_state = self.sensor_value_addition(r_state, input_diff, self.device)
        
        reduced_lidar = torch.cat((r_state, state[-4:].to(self.device)), dim=0).to(self.device)

        action = self.imit_model.actor(reduced_lidar, False)
