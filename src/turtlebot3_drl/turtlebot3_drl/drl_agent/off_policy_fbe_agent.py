...
class OffPolicyAgent(ABC):
    def __init__(self, device, simulation_speed):
        ...
        # Network structure
        self.state_size         = NUM_SCAN_SAMPLES + 4 # Could add more states on environment.py(also environment_real.py)
        self.action_size        = ACTION_SIZE
        self.hidden_size        = HIDDEN_SIZE
        self.map_size           = 128 * 128 * 3
    ...
    def create_network(self, type, name):
        network = type(name, self.map_size, self.input_size, self.action_size, self.hidden_size).to(self.device)
