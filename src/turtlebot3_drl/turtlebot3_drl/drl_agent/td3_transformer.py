...
if ENABLE_IMITATE_ACTION:
    from .ddpg_imitate import DDPG_Imitate
...
class Actor(Network):
    def __init__(self, name, state_size, action_size, hidden_size):
        ...
        self.transformer = Transformer(src_pad_idx=0, trg_pad_idx=0, trg_sos_idx=1, enc_voc_size=state_size, dec_voc_size=state_size, d_model=hidden_size,
                                        n_head=2, max_len=state_size, ffn_hidden=int(hidden_size // 2), n_layers=2, drop_prob=0.3, device=self.gpu)
    ...
    def forward(self, states, visualize=False):
        ...
        if len(states.size()) == 1:
            ...
            is_forward = True
        # else:
        #     states = states.unsqueeze(0).unsqueeze(1)
        batch_size = states.size(0)
        trg = torch.full((batch_size, 1), self.transformer.trg_sos_idx, dtype=torch.long, device=self.gpu)
        transformer_output = self.transformer(states, trg)
        x = self.silu(self.fc1(torch.flatten(transformer_output, start_dim= 1)))
