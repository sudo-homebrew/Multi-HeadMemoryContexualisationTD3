...
from ..common.fbe_settings import ENABLE_VISUAL, ENABLE_STACKING, OBSERVE_STEPS, MODEL_STORE_INTERVAL, GRAPH_DRAW_INTERVAL, ENABLE_MANUAL_ACTION

from ..common.storagemanager_fbe import StorageManager
...
from .td3_fbe import TD3

from turtlebot3_msgs.srv import DrlFbeStep, Goal
from std_srvs.srv import Empty

import rclpy
from rclpy.node import Node
from ..common.replaybuffer_fbe import ReplayBuffer
...
class DrlAgent(Node):
    def __init__(self, training, algorithm, load_session="", load_episode=0, real_robot=0):
        ...
        self.step_comm_client = self.create_client(DrlFbeStep, 'step_comm')
    ...
    def process(self):
        ...
        while (True):
            ...
            action_past = [0.0, 0.0]
            map, state = util.init_episode(self)
            ...
            while not episode_done:
                if self.training and self.total_steps < self.observe_steps:
                    if ENABLE_MANUAL_ACTION:
                        action = self.model.get_action_manual()
                    else:
                        action = self.model.get_action_random()
                else:
                    action = self.model.get_action(map, state, self.training, step, ENABLE_VISUAL)
                ...
                # Take a step
                next_map, next_state, reward, episode_done, outcome, distance_traveled = util.step(self, action_current, action_past)
                ...
                if self.training == True:
                    self.replay_buffer.add_sample(map, state, action, [reward], next_state, next_map, [episode_done])
