...
from ..common.settings import ENABLE_VISUAL, ENABLE_STACKING, OBSERVE_STEPS, MODEL_STORE_INTERVAL, GRAPH_DRAW_INTERVAL, ENABLE_MANUAL_ACTION, MQ_SIZE
...
from ..common.MQ_replaybuffer import ReplayBuffer
...
class DrlAgent(Node):
    ...
    def process(self):
        ...
        while (True):
            ...
            action_past = [0.0, 0.0]
            state = self.state_filter(util.init_episode(self))
            mq = np.zeros((MQ_SIZE, len(state)))
            ...
            print(self.model.step_time)

            while not episode_done:
                if self.training and self.total_steps < self.observe_steps:
                    if ENABLE_MANUAL_ACTION:
                        action, next_mq = self.model.get_action_manual()
                    else:
                        action, next_mq = self.model.get_action_random()
                else:
                    action, next_mq = self.model.get_action(state, self.training, step, ENABLE_VISUAL)
                ...
                if self.training == True:
                    self.replay_buffer.add_sample(mq, state, action, [reward], next_state, next_mq, [episode_done])
    ...
    def state_filter(self, state):
        cat_axis = 0
        lidar_states = state[:360]#NUM_SCAN_SAMPLES]
        env_states = state[-4:]
        lidar_states = (np.exp((np.ones_like(lidar_states) - lidar_states) * 4) - 1) / (np.exp(np.ones_like(lidar_states) * 4) - 1)
        max_values = np.max(lidar_states, axis=cat_axis)
        max_indices = np.argmax(lidar_states, axis=cat_axis)
        max_indices = max_indices / NUM_SCAN_SAMPLES
        max_val_index = np.stack((max_values, max_indices), axis=cat_axis)
        state = np.concatenate((lidar_states, max_val_index, env_states), axis=cat_axis)
