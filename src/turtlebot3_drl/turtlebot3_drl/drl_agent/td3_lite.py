...
class Actor(Network):
    def __init__(self, name, state_size, action_size, hidden_size):
        ...
        self.fa2 = nn.Linear(hidden_size, 30)

        self.fa3 = nn.Linear(55, hidden_size)
        self.fa4 = nn.Linear(hidden_size, int(hidden_size / 2))
        self.fa5 = nn.Linear(int(hidden_size / 2), action_size)
        ...
        self.filter1 = torch.tensor([[[-1.0, 1.0, -1.0, 1.0]]]).to(device=self.device, dtype=torch.float32)
        self.filter2 = torch.tensor([[[-1.0, -1.0, 1.0, 1.0]]]).to(device=self.device, dtype=torch.float32)

        self.conv_fc = nn.Linear(fc_size, 15)

        # Env NN
        self.opfa1 = nn.Linear(6, int(hidden_size / 2 ** 2))
        self.opfa2 = nn.Linear(int(hidden_size / 2 ** 2), 10)

        self.dropout = nn.Dropout(0.3)

        self.layer_norm1 = nn.LayerNorm(hidden_size)
        self.layer_norm1_1 = nn.LayerNorm(int(hidden_size / 3))
        ...
        self.layer_norm5 = nn.LayerNorm(30)
        self.layer_norm6 = nn.LayerNorm(action_size)
    ...
    def forward(self, states, visualize=False):
        ...
        x = self.layer_norm1(self.fa3(x))
        x = self.silu(x)
        x = self.layer_norm2(self.fa4(x))
        x = self.silu(x)
        action = torch.tanh(self.fa5(x))
    ...
    def feature_extraction(self, lidar_state):
        ...
        if cat_dim == 1:
            output = self.silu(torch.cat((output1[:, 0, :], output2[:, 0, :]), cat_dim))
        else:
            output = self.silu(torch.cat((output1[0, 0, :], output2[0, 0, :]), cat_dim))
