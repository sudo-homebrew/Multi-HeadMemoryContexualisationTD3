...
class Actor(Network):
    def __init__(self, name, state_size, action_size, hidden_size):
        ...
        # --- define layers here ---
        state_size = 364
        # self.gru = nn.GRU(input_size=state_size, hidden_size=hidden_size, num_layers=1, batch_first=True, bidirectional=True)
        ...
        self.fa4 = nn.Linear(int(hidden_size * 2), hidden_size)
    ...
    def forward(self, states, visualize=False):
        ...
        x6 = torch.sigmoid(self.fa6(x5))
        x7 = torch.sigmoid(self.fa7(x6))
        x8 = self.fa8(x7)
        action = torch.tanh(x8)

        # -- define layers to visualize here (optional) ---
        if visualize and self.visual:
            self.visual.update_layers(states, action, [x1, x2, x3, x4, x5, x6, x7],
                                    [self.fa1.bias, self.fa2.bias, self.fa3.bias, self.fa4.bias,
                                    self.fa5.bias, self.fa6.bias, self.fa7.bias])
...
class DDPG_Imitate(OffPolicyAgent):
