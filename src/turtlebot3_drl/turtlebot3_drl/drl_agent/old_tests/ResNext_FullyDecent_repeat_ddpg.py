...
# ResNext_FullyDecent_repeat_ddpg.py
...
class Actor(Network):
    def __init__(self, name, state_size, action_size, hidden_size):
        ...
        self.fa2 = nn.Linear(ResNext_output_size, int(ResNext_output_size / 2))
        self.fa3 = nn.Linear(int(ResNext_output_size / 2 ** 1), int(ResNext_output_size / 2 ** 2))
        self.fa4 = nn.Linear(int(ResNext_output_size / 2 ** 2), int(ResNext_output_size / 2 ** 3))
        self.fa5 = nn.Linear(int(ResNext_output_size / 2 ** 3), int(ResNext_output_size / 2 ** 4))
        self.fa6 = nn.Linear(int(ResNext_output_size / 2 ** 4), int(ResNext_output_size / 2 ** 5))
        self.fa7 = nn.Linear(int(ResNext_output_size / 2 ** 5), int(ResNext_output_size / 2 ** 6))
        self.fa8 = nn.Linear(int(ResNext_output_size / 2 ** 6), int(ResNext_output_size / 2 ** 7))
        self.fa9 = nn.Linear(int(ResNext_output_size / 2 ** 7), int(ResNext_output_size / 2 ** 8))
        self.fa10 = nn.Linear(int(ResNext_output_size / 2 ** 8), action_size)
...
#        self.apply(super().init_weights)
    ...
    def forward(self, states, visualize=False):
        ...
        x5 = torch.relu(self.fa5(x4))
        x6 = torch.relu(self.fa6(x5))
        x7 = torch.relu(self.fa7(x6))
        x8 = torch.relu(self.fa8(x7))
        x9 = torch.relu(self.fa9(x8))
        action = torch.tanh(self.fa10(x9))
