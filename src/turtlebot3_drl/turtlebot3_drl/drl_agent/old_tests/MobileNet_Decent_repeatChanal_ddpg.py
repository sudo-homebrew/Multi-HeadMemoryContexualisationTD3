...
# MobileNet_Decent_repeatChanal_ddpg.py
...
class Actor(Network):
    def __init__(self, name, state_size, action_size, hidden_size):
        ...
        # --- define layers here ---
        self.fa1 = mobilenet_v3_small(pretrained=True).to(device)
        self.fa1.classifier[3] = nn.Linear(in_features=self.fa1.classifier[3].in_features, out_features=outfeature_size)
        self.fa2 = nn.Linear(outfeature_size, int(outfeature_size / 2))
        self.fa3 = nn.Linear(int(outfeature_size / 2), int(outfeature_size / 4))
        self.fa4 = nn.Linear(int(outfeature_size / 4), int(outfeature_size / 8))
        self.fa5 = nn.Linear(int(outfeature_size / 8), int(outfeature_size / 16))
        self.fa6 = nn.Linear(int(outfeature_size / 16), action_size)
...
#        self.apply(super().init_weights)
    ...
    def forward(self, states, visualize=False):
        ...
        if states.dim() == 1:
            reshaped_states = states.view(1, 1, 28, 13).repeat(1, 3, 1, 1)
            x1 = torch.relu(self.fa1(reshaped_states))[0]
        if states.dim() == 2:
            reshaped_states = states.view(BATCH_SIZE, 1, 28, 13).repeat(1, 3, 1, 1)
