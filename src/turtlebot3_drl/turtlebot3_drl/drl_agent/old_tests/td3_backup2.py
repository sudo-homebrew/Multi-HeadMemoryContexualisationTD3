...
class Actor(Network):
    def __init__(self, name, state_size, action_size, hidden_size):
        ...
        self.fa7 = nn.Linear(int(hidden_size / 2 ** 2), int(hidden_size / 2 ** 2))
        self.fa8 = nn.Linear(int(hidden_size / 2 ** 2), int(hidden_size / 2 ** 3))
        self.fa9 = nn.Linear(int(hidden_size / 2 ** 3), action_size)

        self.dropout = nn.Dropout(0.3)

        self.layer_norm1 = nn.LayerNorm(hidden_size)
        self.layer_norm2 = nn.LayerNorm(int(hidden_size / 2 ** 2))
    ...
    def forward(self, states, visualize=False):
        ...
        x = torch.sigmoid(self.fa6(x))
        x = torch.relu(self.fa7(x))
        x = torch.sigmoid(self.fa8(x))
        action = torch.tanh(self.fa9(x))
