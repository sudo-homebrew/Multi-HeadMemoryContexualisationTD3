...
class Actor(Network):
    def __init__(self, name, state_size, action_size, hidden_size):
        ...
        self.fa1.fc = nn.Linear(in_features=self.fa1.fc.in_features, out_features=action_size)
...
#        self.apply(super().init_weights)
    ...
    def forward(self, states, visualize=False):
        ...
        if states.dim() == 1:
            reshaped_states = states.view(1, 3, 61, 2)
#            reshaped_states = reshaped_states.repeat(BATCH_SIZE, 1, 1, 1)
            action = torch.tanh(self.fa1(reshaped_states))[0]
        if states.dim() == 2:
            reshaped_states = states.view(BATCH_SIZE, 3, 61, 2)
            # --- define forward pass here ---
            action = torch.tanh(self.fa1(reshaped_states))
