...
# TD3 concatiation without finetuned
...
class Actor(Network):
    def __init__(self, name, state_size, action_size, hidden_size):
        ...
        self.conv_fc = nn.Linear(fc_size, int(hidden_size / 2 ** 2))

        # Env NN
        self.opfa1 = nn.Linear(6, int(hidden_size / 2 ** 2))
        self.opfa2 = nn.Linear(int(hidden_size / 2 ** 2), int(hidden_size / 2 ** 2))
        # self.opfa3 = nn.Linear(int(hidden_size / 2 ** 2), int(hidden_size / 2 ** 3))

        self.dropout = nn.Dropout(0.5)

        self.layer_norm1 = nn.LayerNorm(hidden_size)
        self.layer_norm1_1 = nn.LayerNorm(int(hidden_size / 2))
