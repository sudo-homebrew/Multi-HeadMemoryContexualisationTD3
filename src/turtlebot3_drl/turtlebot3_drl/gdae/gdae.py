...
class ImplementEnv(Node):
    ...
    def cycle(self):
        while True:
            ...
            self.step(self.sub.scanMsg.ranges, self.sub.scanMsg.angle_increment, self.sub.last_pose_x, self.sub.last_pose_y, self.sub.last_pose_theta, self.sub.map, self.sub.tf_buffer)
            
            poi_node = rclpy.create_node("markers")
            
#            map_poi = self.desired_points_callback_rev(self.poi, self.sub.tf_buffer)
            map_poi = self.poi

            print("map_poi---------------", map_poi)
            marker7 = Marker()
            marker7.header.frame_id = "/map"
            marker7.type = Marker.CUBE
            marker7.action = Marker.ADD
            marker7.id = 1000
            marker7.pose.position.x = map_poi[0]
            marker7.pose.position.y = map_poi[1]
            marker7.pose.position.z = 0.0
            ...
            marker_pub7 = poi_node.create_publisher(Marker, 'poi', 10)
            marker_pub7.publish(marker7)

            node_node = rclpy.create_node("markers")
            for i in range(0, len(self.node)):
                ...
                globals()["marker{}".format(i)].pose.position = Point(x=self.node[i][0], y=self.node[i][1], z=0.0)
                ...
                globals()["marker_pub{}".format(i)] = node_node.create_publisher(Marker, 'node', 10)
                #for i in range(0, 2):
...
#        theta -= math.pi / 2
        ...
        x_cordinate, y_cordinate = calcqxqy(dist - 0.3, theta, pos_twist)
        
        return x_cordinate + pos_x, y_cordinate + pos_y
    


    def delete_nodes(self, pos_x, pos_y):
        self.del_dup_node()

        for index, element in enumerate(self.node):
            if self.dist(element[0], element[1], pos_x, pos_y) < 0.5 or self.dist(element[0], element[1], self.poi[0], self.poi[1]) < 0.5:
                del self.node[index]
    ...
    def del_dup_node(self):
        for index, element in enumerate(self.node):
            for i, e in reversed(list(enumerate(self.node))):
                if self.dist(element[0], element[1], e[0], e[1]) < 0.5 and index != i:
                    del self.node[index]
                    break
    ...
    def desired_points_callback(self, point, transform_listener): # msg tyep : PointStamped
        try:
            ...
            transform_msg = transform_listener.lookup_transform('odom', 'map', time = rclpy.time.Time())
            transformed_points = self.transform_points(p, transform_msg)
#            self.publish_markers(transformed_points)
#            print(transformed_points)
            return [transformed_points.x, transformed_points.y]
...
#            logging.error('Error occurred: {}'.format(str(e)))
    ...
    def desired_points_callback_rev(self, point, transform_listener): # msg tyep : PointStamped
        try:
            if self.reference_point is None:
                self.reference_point = [0.0, 0.0]  # Initial point of robot
            ...
            transform_msg = transform_listener.lookup_transform('map', 'odom', time = rclpy.time.Time())
            transformed_points = self.transform_points_rev(p, transform_msg)
#            self.publish_markers(transformed_points)
#            print(transformed_points)
            return [transformed_points.x, transformed_points.y]
        except Exception as e:
#            logging.error('Error occurred: {}'.format(str(e)))
    ...
    def transform_points_rev(self, point, transform_msg):
        point_stamped = PointStamped()
    ...
    def map_information(self, candidate_x, candidate_y, k, map, transform_listener):
        try:
            m = map.data
            d = np.array(m.tolist())
#            d = np.where(d < 50, 5, d).tolist()
#            d = np.where(d > 253, 1, d).tolist()
#            d = np.where(d > 50, 0, d).tolist()
            d[d < 50] = 5 # obstacle
            d[d >= 253] = 1 # free
            d[d >= 50] = 0 # unknown
        ...
        for w in range(-int(k/2), int(k/2)):
            for h in range(-int(k/2), int(k/2)):
                ...
                sum1 = m_value[int(map_pont_x)][int(map_pont_y)]
                
        tem = sum1 / (k**2)
#        print('map information has called')
        return math.e ** tem
...
def main(args=sys.argv[1], args1=sys.argv[2]):
    ...
    goal_node = rclpy.create_node("markers")

    marker9 = Marker()
    marker9.header.frame_id = "/map"
    marker9.type = Marker.CUBE
    marker9.action = Marker.ADD
    marker9.id = 1000
    marker9.pose.position.x = pose[0]
    marker9.pose.position.y = pose[1]
    marker9.pose.position.z = 0.0
    #marker7.pose.orientation.w = 1.0
    marker9.scale.x = 0.2
    marker9.scale.y = 0.2
    marker9.scale.z = 0.01
    marker9.color.a = 1.0
    marker9.color.r = 1.0
    marker9.color.g = 0.0
    marker9.color.b = 0.0

    marker_pub9 = goal_node.create_publisher(Marker, 'goal', 10)
    marker_pub9.publish(marker9)
