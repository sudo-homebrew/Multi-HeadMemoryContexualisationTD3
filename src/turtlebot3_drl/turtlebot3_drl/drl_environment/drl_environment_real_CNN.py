...
# Also for using Lidar sensor LDS-02
...
class DRLEnvironment(Node):
    def __init__(self, is_custom_service = True):
        ...
        self.is_custom_service_using = is_custom_service
    ...
    *******************************************************************************"""
    ...
    def scan_callback(self, msg):
        ...
        msg.ranges = self.filtered_lidar(msg.ranges)
    ...
    def sensor_value_addition(self, lidar_sensor, input_diff):
        insert_gap = int(abs(len(lidar_sensor) / input_diff))
        for i in range(input_diff - 1, -1, -1):
            idx = insert_gap * i
            lidar_sensor.insert(idx + 1, (lidar_sensor[idx] + lidar_sensor[idx + 1]) / 2)
        return lidar_sensor

    def sensor_value_deletion(self, lidar_sensor, input_diff):
        del_gap = int(abs(len(lidar_sensor) / input_diff))
        for i in range(input_diff - 1, -1, -1):
            idx = del_gap * i
            del lidar_sensor[idx]
        return lidar_sensor

    def filtered_lidar(self, lidar):
        filtered_lidar = []
        for lidar_val in lidar:
            if numpy.isnan(lidar_val):
                filtered_lidar.append(REAL_LIDAR_DISTANCE_CAP)
            else:
                filtered_lidar.append(lidar_val)

        input_diff = len(filtered_lidar) - REAL_N_SCAN_SAMPLES
        
        if input_diff < 0:
            lidar_sensor_input = self.sensor_value_addition(filtered_lidar, abs(input_diff))
        elif input_diff > 0:
            lidar_sensor_input = self.sensor_value_deletion(filtered_lidar, abs(input_diff))
        else:
            lidar_sensor_input = lidar
        return lidar_sensor_input
    ...
    def stop_reset_robot(self, success):
        ...
        if success:
            if self.is_custom_service_using:
            ...
            twist = Twist()
            twist.linear.x = -(REAL_SPEED_LINEAR_MAX)
    ...
    def get_state(self, action_linear_previous, action_angular_previous):
        ...
        state.append(float(min(self.scan_ranges)))
        state.append(float(self.scan_ranges.index(min(self.scan_ranges)) / REAL_N_SCAN_SAMPLES))
...
def main(args=sys.argv[1:]):
    ...
    if len(args) == 0:
        drl_environment = DRLEnvironment(is_custom_service = True)
    elif len(args) == 1:
        drl_environment = DRLEnvironment(is_custom_service = False)
    else:
        rclpy.shutdown()
