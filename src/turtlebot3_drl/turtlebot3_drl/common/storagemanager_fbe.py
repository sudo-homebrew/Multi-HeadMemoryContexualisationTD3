...
class StorageManager:
    def __init__(self, name, load_session, load_episode, device, stage):
        if load_session and name not in load_session:
            ...
            while True:
                pass
        self.machine_dir = (os.getenv('DRLNAV_BASE_PATH') + '/src/turtlebot3_drl/fbe_model/' + str(socket.gethostname()))
        if 'examples' in load_session:
            self.machine_dir = (os.getenv('DRLNAV_BASE_PATH') + '/src/turtlebot3_drl/fbe_model/')
        self.name = name
        is_stage_10 = load_session[-2:] if load_session else stage
        if is_stage_10 == '10' or is_stage_10 == 'NN':
            self.stage = 10
        else:
